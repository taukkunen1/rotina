(() => {
  ...
  async function loadRemote() {
    try {
      ...
      data = { config: {}, state: {}, revision: 0, serverUpdatedAt: null };
  ...
  function repairEditorConfig() {
    ...
    if (!cfg.periods || typeof cfg.periods !== 'object') cfg.periods = {};
    const defaults = {
      manha: { label:'Manhã', time:'8:00 – 12:00' },
      tarde: { label:'Tarde', time:'12:00 – 18:00' },
      noite: { label:'Noite', time:'18:00 – 22:00' }
    };
    Object.keys(defaults).forEach(k => {
      if (!cfg.periods[k] || !Array.isArray(cfg.periods[k].tasks)) cfg.periods[k] = { ...defaults[k], tasks: [] };
      if (!cfg.periods[k].label) cfg.periods[k].label = defaults[k].label;
      if (!cfg.periods[k].time) cfg.periods[k].time = defaults[k].time;
    });
    if (cfg.periodsWeekend) Object.keys(defaults).forEach(k => {
      if (!cfg.periodsWeekend[k] || !Array.isArray(cfg.periodsWeekend[k].tasks)) cfg.periodsWeekend[k] = { ...cfg.periods[k] };
    });
    ...
    if (cfg.editorPin == null) cfg.editorPin = '';
    if (cfg.screenDailyLimitHours == null) cfg.screenDailyLimitHours = 2;
    data.config = cfg;
  }
  ...
  function exportBackup() {
    const snapshot = { config: data.config, state: data.state, revision: data.revision, serverUpdatedAt: data.serverUpdatedAt, exportedAt: new Date().toISOString() };
    const blob = new Blob([JSON.stringify(snapshot, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'rotina-backup.json'; a.click();
  ...
  async function importBackup(file) {
    ...
    setStatus('Enviando backup para o servidor…');
    const result = await storage.replaceRemote(imported);
    data = { config: imported.config || {}, state: imported.state || {}, revision: Number(result.revision || 0), serverUpdatedAt: result.serverUpdatedAt || null };
    storage.writeJSON(storage.CONFIG_KEY, data.config);
    storage.writeJSON(storage.STATE_KEY, data.state);
    render();
    setStatus('Backup restaurado no servidor');
  }
  ...
  async function start() {
    ...
    $('import-backup').addEventListener('change', async e => {
      ...
      catch (err) { console.error(err); setStatus('Erro ao restaurar backup no servidor'); }
      e.target.value = '';
    });
