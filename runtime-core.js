(() => {
    ...
    try{
      if(typeof DEFAULT_CONFIG !== 'undefined' && typeof CONFIG !== 'undefined'){
        ...
        ['manha','tarde','noite'].forEach(key => {
          if(!CONFIG.periods[key] || !Array.isArray(CONFIG.periods[key].tasks) || !CONFIG.periods[key].tasks.length){
            CONFIG.periods[key] = clone(DEFAULT_CONFIG.periods[key]);
          }
        });
      ...
      console.error('PACUS normalize failed', error);
    }
  }
  ...
  function repairConfig(){
    ...
    if(!CONFIG.pet || typeof CONFIG.pet !== 'object'){
      ...
      if(!Array.isArray(CONFIG.pet.stages) || CONFIG.pet.stages.length < 2){ CONFIG.pet.stages = clone(defaults.pet.stages); changed = true; }
    }
    ...
    if(changed && typeof saveConfig === 'function'){
      try{ saveConfig(CONFIG); }catch(error){ console.warn('PACUS: não foi possível salvar a configuração reparada.', error); }
    }
    return changed;
  }
  ...
  function clearCrashBannerIfHealthy(){
    ...
    if(periods && periods.children.length > 0){
      document.getElementById('appCrashBanner')?.remove();
  ...
  function recover(){
    try{
      ...
      if(!clearCrashBannerIfHealthy()){
        try{ render(); }catch(error){ console.error('PACUS runtime recovery:', error); }
        clearCrashBannerIfHealthy();
      }
    }catch(error){
      console.error('PACUS runtime recovery failed:', error);
    }
  }

  function install(){
    normalize();
    if(typeof window.render === 'function' && !window.__pacusGuardV3){
      const original = window.render;
      window.__pacusGuardV3 = true;
      window.render = function(){ normalize(); return original.apply(this, arguments); };
    }
    try{ if(typeof saveState === 'function') saveState(state); }catch(error){}
    try{ if(typeof saveConfig === 'function') saveConfig(CONFIG); }catch(error){}
    try{ if(typeof render === 'function') render(); }catch(error){ console.error('PACUS guarded render failed', error); }
  }
